...
const FoodPartnerLogin = () => {
    ...
    const isUserActive = location.pathname === '/user/login';
            ...
            <div className='bg-linear-to-b from-sky-950 via-slate-900 to-gray-900 w-full max-w-md px-6 py-6 rounded-lg border-2 border-solid border-gray-700 shadow-xl shadow-black'>
                ...
                <nav className='flex justify-center mt-0.5'>
                    <div className='inline-flex rounded-full border border-slate-600 bg-slate-800 p-1'>
                        <Link className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors duration-200 ${isUserActive ? 'bg-blue-500 text-white' : 'text-blue-400 hover:text-blue-300'}`} to='/user/login'>
                            User
                        </Link>

                        <Link className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors duration-200 ${!isUserActive ? 'bg-blue-500 text-white' : 'text-blue-400 hover:text-blue-300'}`} to='/food-partner/login'>
                            Food Partner
                        </Link>                        
                    </div>
                </nav>
                ...
                <form onSubmit={handleSubmit} className='flex flex-col gap-4 mt-2.5'>
                    ...
                    <div className='flex flex-col gap-1'>
                        ...
                        <input className='border-2 border-solid border-slate-600 bg-slate-700 py-2 px-2 w-full rounded-sm text-slate-100 outline-none min-w-0' type="email" id="email" name='email' placeholder='business@example.com' required autoComplete='email' />
                    </div>
                    ...
                    <div className='flex flex-col gap-1'>
                        ...
                        <div className='relative'>
                            <input className='border-2 border-solid border-slate-600 bg-slate-700 py-2 px-2 w-full rounded-sm text-slate-100 outline-none min-w-0' type={showPassword ? 'text' : 'password'} id="password" name='password' placeholder='••••••••' required autoComplete='current-password' />
                ...
                <div className='text-center mt-4 text-slate-400'>
                    New Partner?
                    <Link className='text-blue-600 hover:text-blue-400 transition-colors duration-200' to='/food-partner/register'>{' '}Sign Up</Link>
                </div>
            </div>
        </div>
    );    
